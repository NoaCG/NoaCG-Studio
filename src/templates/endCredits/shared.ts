...
/**
 * THE CREDITS PARSER, emitted verbatim into every template that reads the one pasted field: the
 * catalog's credit rolls below, and an imported SVG whose Credits text carries the same list
 * (templates/importedDesign/creditsRoll.ts). One source, so the format docs/END_CREDITS.md
 * teaches is one format wherever the operator pastes it. Plain text inside the literal - no
 * interpolation, no backtick - because scripts/credits-parser.test.mjs cuts this block out of
 * the source and runs it as the JavaScript a template ships.
 *
 * `parseCredits(text, escape)`: `escape` is the function every value passes through ON THE
 * WAY OUT. The catalog designs leave it off and get `escapeHtml`, because their row builders
 * concatenate into innerHTML; a renderer that writes textContent (SVG rows) passes the identity.
 */
export const CREDITS_PARSER_JS = `// parseCredits(text): the whole credit list is ONE pasted block of plain text, and there is
...
function parseCredits(text, escape) {
  // The one boundary every value crosses - see the note above this block.
  ...
  function openGroup(role) {
    group = { type: 'group', role: esc(role.trim()), names: [] };
    current.push(group);
  }
  ...
  text.split('\\n').forEach(function (raw) {
    ...
    if (line.charAt(0) === '#') {                        // "# PRODUCTION" -> department heading
      ...
      current.push({ type: 'heading', text: esc(line.slice(1).trim()) });
      return;
    }
    ...
    if (split.length >= 2) {
      openGroup(split[0]);
      group.names.push(esc(split.slice(1).join(' ').trim()));
      return;
    }
    ...
    if (mark > 0 && mark <= ROLE_LABEL_MAX) {
      ...
      if (inlineName) group.names.push(esc(inlineName));
      return;
    }
    ...
    if (group) group.names.push(esc(line));
    else current.push({ type: 'entry', text: esc(line) });
  });
  pushSection();
  return sections;
}
