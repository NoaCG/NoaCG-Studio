...
// The ANIMATION markers and the two readers of that region live in blocks/, beside the
// importer they describe - `bridgeApi.normalize` needs the same explanation this lane does.
import { ANIMATION_CLOSE, ANIMATION_OPEN, animationRange } from '../../../blocks/animationRegion.js';
...
export function describeWritableRegions(template: SpxTemplate, prefix: string, maxChars = 6000): string {
  ...
  return [
    ...
    clip(designCss),
    '',
    '=== animation (yours; the authoring grammar) ===',
    'REQUIRED, in this order and this exact form, or the importer reads none of it:',
    "  var animSpeed = 1;   var easeIn = 'expo.out';   var easeOut = 'power3.in';   (eases SINGLE-quoted, each var on its own line)",
    '  function buildInTimeline() { … }   function buildOutTimeline() { … }   (each returns one gsap.timeline(); each closing } starts its own line)',
    '  inside them: tl.set / tl.to / tl.fromTo with literal values only, durations as N / animSpeed, no DOM measurement.',
    clip(animText),
  ].join('\n');
}
