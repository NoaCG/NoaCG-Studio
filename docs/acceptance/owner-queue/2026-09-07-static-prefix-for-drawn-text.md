...
docs/acceptance/OWNER_QUEUE.md names this item as its worked example of what is NOT his: 'whether
...
## Decided 2026-09-10 - `static:` stays, and this item is the contract's own worked example

`docs/acceptance/OWNER_QUEUE.md` names this exact question in the passage that explains why the
`because:` key exists:

> An agent walking the computer list called three import items taste. Two of them were not: which
> share of the artwork makes a layer a background plate is answered by the distribution already in
> the code, and whether a prefix reads `static:` or `d:` is a naming default.

So the prefix is decided, in the direction the item already argued: **`static:` stays.** It says
what it means to a designer who has never read our docs, it cannot be misread as `f:` at a glance,
and `d:` is opaque unless you already know it stands for "drawing". A one-letter prefix saves four
characters in a layer name and costs a student the guess.

**And the second question goes the same way: the unticked row keeps saying `stays as drawn`.** The
item worried it reads identically to a row the reader unticked themselves. It should - the state is
identical, and the row says what the graphic will do rather than who decided it. A label that
explained its own provenance would be telling the reader about the import instead of about their
graphic.

What is left here is a claim an agent drives: ten Rank rows arrive off, only Title is ticked, and
the operator's boxes are two rather than twelve.
