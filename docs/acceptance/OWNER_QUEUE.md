...
- 2026-09-09-s-the-25-september-deck-opens-in-powerpoint - into
  `2026-09-10-ca-the-deck-now-ends-at-our-own-player.md`. The deck rebuild filed a second walk
  item for the same file, so the owner had two items opening one deck. The first one's surviving
  question, its `Create project` decision and its font note are carried verbatim in the second.
  Both were `walk` items and the absorbing one was not re-kinded, so the caveat below about
  folding into an `agent` item does not apply.
