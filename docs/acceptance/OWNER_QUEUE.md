...
- 2026-09-09-g, the venue network-diagnostics screenshot - dropped 2026-09-10 on his ruling: he owns getting
  the venue network screenshot and does not want reminding. The fallback if he cannot is
  unchanged: we bring our own connection and say so. **The item asked for two things and he
  answered one.** Its second question, which OGraf renderer the receiving broadcaster runs, was re-filed before the
  delete as `2026-09-10-be-which-ograf-renderer-yle-runs.md` and is still open - so if he meant
  to drop that one too, deleting that file is what does it.

### 2026-09-11 - one consolidation, no drops

- Two walk items opening the same presentation file were consolidated into one, so the owner had
  one item rather than two. The first one's surviving question, its `Create project` decision and
  its font note were carried verbatim into the second. Both were `walk` items and the absorbing
  one was not re-kinded, so the caveat below about folding into an `agent` item does not apply.
  Both were deleted on 2026-09-20 with the rest of the teaching-session material.
...
- 2026-09-06-puzzle-board-letters - into `2026-09-06-puzzle-reveal-letter-press.md`. Its one
  ...
  board, the same week.
- A slide-level presentation item was folded into the item that opened the same file at the same
  double-click, its surviving question travelling on with it. Both were deleted on 2026-09-20
  with the rest of the teaching-session material.
...
> whatever they are doing, we need to catch up. We need to have a graphic creator that can play
> out graphics I can use myself and, one day, the rest of the world can too.
