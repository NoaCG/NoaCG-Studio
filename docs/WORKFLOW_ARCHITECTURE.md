...
*Owner decision 2026-09-06: the organization is not decided yet, so phase 1 starts with the Actions lander and is built so the merge queue is a later configuration change, not a second mechanism.* `land.yml` triggered by the `land` label or `workflow_dispatch`, `concurrency: {group: landing, cancel-in-progress: false}`: checkout `main`, merge the PR head, run the same `T2 gate` job `ci.yml` will later run on `merge_group`, `git push origin HEAD:main`, remove the label; on failure comment and unlabel. Sequential only, ~8 min per landing, about 180 a day. A ruleset (rulesets work on user-owned public repos) restricts pushes to `main` to the Actions identity so no second lander can exist, and requires the `noacg/reviewed` status. The client command, the stamp, T3 and the orchestrator's view are identical in both modes; switching later means enabling the queue in the ruleset, adding `merge_group` to `ci.yml`'s triggers and changing one line in the client from "label" to `gh pr merge --auto`. Nothing built for the lander is thrown away.
