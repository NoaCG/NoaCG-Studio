...
*How each tool reaches the rest (measured 2026-09-26 with `scripts/instruction-load-probe.mjs`).* Claude: `.claude/settings.json` sets the built-in agents-md plugin to `claude-md`, so Claude reads only `CLAUDE.md` files - the root import at start, a hand-written folder contract through its `CLAUDE.md` wrapper on the first read in that folder, and generated rules through `.claude/rules/` on the read of a file they scope to. Without that setting the desktop app injected every generated folder contract on top of its `.claude/rules/` copy. Codex: `~/.codex/AGENTS.md` and the `AGENTS.md` chain from the repository root down to the folder it started in, once, at start; it never reads `CLAUDE.md` or `.claude/rules/`, so for a file elsewhere the root contract tells it to run `npm run rules -- <path>`. Each rule lives in one folder contract per chain (`ruleHomes`), so no chain carries a rule twice.
