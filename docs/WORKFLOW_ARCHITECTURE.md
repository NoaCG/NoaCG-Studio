...
*The kernel is TWO numbers, not one, and §5.3's "Kernel `AGENTS.md` (≤ 8 KB)" conflated them - corrected 2026-09-08 after an audit read the plan against the tree.* **The kernel proper is `.claude/rules/everywhere.md`**, the one compiled file with no `paths:` scope, so every Claude session pays for it before touching anything: 7,852 bytes of 8,192, and the compile refuses it over. That is the number the 8 KB ceiling was always about, and it is met. **The generated root `AGENTS.md` is a different and larger file** - 15,841 bytes - because it carries the kernel PLUS every rule whose scope spans two trees and therefore has no owner below the root. Those rules have nowhere better: a rule binding `src/templates/**` and `e2e/**` is only visible to both from the root, and Codex reads `AGENTS.md` files and nothing else. So the root is not a target to drive under 8 KB; the way to shrink it is to give a shared ancestor its own contract, which moves those rules DOWN. `src/AGENTS.md` is the first of those, and §9's kernel row must name which file it measures, because the 22,733-byte baseline was taken from the root and the ceiling is checked on `everywhere.md`.
