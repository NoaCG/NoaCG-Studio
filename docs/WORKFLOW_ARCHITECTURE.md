...
6. The docs steer towards bigger runs: "asks for the full suite" (`AGENTS.md:215`), "must be a FULL DISPATCH, never a bare push" (`queue-merge.md:108`). `VERIFICATION.md` stopped steering that way on 2026-09-16, when the cancelled-predecessor reason for a full dispatch went away with the bug.
...
- *The write path has one destination.* The only honest move for a session that learns something is a paragraph in the nearest contract, and the only removal mechanism is a human. 77 commits from 24 branches touched the root in 30 days; `docs/VERIFICATION.md` is 71 KB, the retired owner rulings 40 KB, `.agent-workflows/orchestrator/incidents.md` tripled in five days. `docs/MISTAKE_TRIGGERS.md`, the file that says where a lesson should go, is itself a 26 KB narrative.
...
What changes the shape: `main` stops running `--all` per landing (T3 is one run per batch, and superseded runs exit); `package.json` leaves `CORE` (a script-line edit is not a dependency change; the selector diffs the `dependencies` keys instead); the two widest `MAP` rules are replaced by the graph-derived map. One `setup` job builds and uploads `dist` and the node_modules cache; shards restore and run. A new spec declares what it covers in a header, and the build fails a spec with neither a header nor a coverage-map entry (the "unmapped spec never runs" backlog item, now a gate).

**A gate says how much it looked at, and zero is a failure.** Every tier above assumes a green gate means the gate ran. On 2026-09-08 three mechanisms passed while measuring nothing, found independently of each other: `type-floor.mjs` resolved its floors by a regex over a constant PR #131 had moved, a rule declaring `fires: test:<spec>` was treated as carried by a mechanism that never printed it, and a contract marker was satisfied by an incident write-up quoting it. A gate now reports the size of the set it resolved with `measured(n, subject)` (`scripts/measured.mjs`), which exits 1 on zero whoever ran the gate; `scripts/gates.mjs` refuses a check that exits 0 having reported nothing, refuses a test file that registered no tests, and `check:gate-coverage` refuses a new gate that neither reports nor declares `// measures: none - <why>`. `docs/metrics/2026-09-08-gates-that-measure-nothing.md` classifies all 138 discovered gates, the three catalog gates the discovery cannot see, and the 17 validators.
...
*Phase 1c, as landed.* A red `main` or merge-group run re-runs its failed spec files once on the same commit (`e2e-retry`, from the shards' blob reports; it refuses when a shard never reported, when nothing failed, or when the change edits the failing spec); a fail-then-pass passes the gate and the `after-gate` job writes the specs into `e2e/quarantine.json` through the merge queue; quarantined specs leave the blocking shards and `quarantine.yml` runs each on every push to `main`, posting `noacg/quarantine/<spec>` as a commit status, and queues the release after `RELEASE_AFTER` consecutive passes read off those statuses; a `main` push whose failure survives the second run, after a green last verdict, gets everything since that verdict reverted on `revert/<sha7>` and queued, and the red-main issue names the pull request (`scripts/e2e-quarantine.mjs`, `scripts/revert-landing.mjs`, `scripts/queue-pr.mjs`; the rules in `docs/VERIFICATION.md`, "A red main answers itself first"). The required check stays a read-only verdict; the writes live in `after-gate`. The mechanical pull requests carry a `noacg/reviewed` stamp whose description says `mechanical:`, and ask for their branch run by dispatch with `require_review`, since a push made with the workflow token starts no run; the organisation must let Actions open pull requests (`scripts/landing-ruleset.mjs --apply`, `needs: account` for the `admin:org` scope).

*Phase 1d, as landed.* The build line names no check and no test: `node scripts/gates.mjs run` discovers every `check:*` script whose entry declares `// gate: build` and every `scripts/**/*.test.mjs` on disk, and ci.yml's Factory gates job runs the `factory` tier (a browser, servers) the same way. Each gate declares its tier (`gate: build | factory | after-build | workflow <file> | none - <why>`) and the paths it guards (`guards:`) in its own header; `check:gate-coverage` is inverted from "does something run this gate" to "is this declaration true": a declared workflow must name the gate, a `none` needs a reason, every guard must match a repository file, every `scripts/check-*.mjs` needs a `check:` script. `node scripts/gates.mjs list --changed <ref>` reports which gates a change reaches; the build still runs the whole tier, and the declarations are what T0 will select from.
...
*The kernel is the generated root `AGENTS.md`, and nothing else (2026-09-25).* It holds only the `**`-scoped rules, every Claude and Codex session loads it (Codex natively, Claude through the root `CLAUDE.md` import), and `KERNEL_MAX_BYTES` caps it at 3 KB, so adding a rule there retires another. The old second kernel, `.claude/rules/everywhere.md`, is gone: it loaded the same rules a second time in every Claude session. A rule whose scope spans two top-level folders no longer falls back to the root; the compiler writes it into each folder's own contract (`ruleHomes` in `scripts/contracts-lib.mjs`), and Codex reaches anything else through `npm run rules -- <path>`. Earlier numbers for this paragraph (7,852 B in everywhere.md, 15,841 B in the root) are history in `docs/metrics/`.

*How each tool reaches the rest (measured 2026-09-26 with `scripts/instruction-load-probe.mjs`).* Claude: `.claude/settings.json` sets the built-in agents-md plugin to `claude-md`, so Claude reads only `CLAUDE.md` files - the root import at start, a hand-written folder contract through its `CLAUDE.md` wrapper on the first read in that folder, and generated rules through `.claude/rules/` on the read of a file they scope to. Without that setting the desktop app injected every generated folder contract on top of its `.claude/rules/` copy. Codex: `~/.codex/AGENTS.md` and the `AGENTS.md` chain from the repository root down to the folder it started in, once, at start; it never reads `CLAUDE.md` or `.claude/rules/`, so the root contract tells it to run `npm run rules -- <files>` before editing, which prints every hand-written folder contract on those paths once and the rules scoped to them (measured 2026-09-26: a root-started Codex ran it once, with every file it planned to touch, and read no folder AGENTS.md besides). Each rule lives in one folder contract per chain (`ruleHomes`), so no chain carries a rule twice.

*Phase 2b, the first row, as landed.* The compiler writes a nested contract as well as the Claude layer. A directory is the compiler's once its `AGENTS.md` carries the generated marker - which is what migrating an area DOES, so there is no second registry saying which areas are done and no way for a registry and the files to disagree. A directory with no contract at all is deliberately NOT owned: recording one rule about `src/store` must not quietly invent `src/store/AGENTS.md`, and a rule spanning two areas must not invent `src/AGENTS.md` in a position forty chains load from. Ownership is read by scanning the tree rather than derived from the rules, so an area whose last rule is deleted stays owned long enough for its stranded contract to be removed, and `--report` names every rule no migrated directory owns - during phase 2b that is almost all of them, and it is how the shrinking corpus stays visible. A migrated directory has no sibling `CLAUDE.md`: Claude Code loads the same rules from `.claude/rules/` when it reads a file they scope to, so a wrapper would charge every Claude session for them twice, and `check-shared-instructions` now refuses one beside a generated contract instead of demanding it. A rule belongs to the deepest directory containing every glob in its scope, and one whose scope reaches the repository root is a KERNEL rule, capped at 8,192 bytes because every session pays for it before touching anything. Each migrated directory carries its own `.gitattributes` naming the `noacg-contracts` merge driver (`scripts/contracts-merge-driver.mjs`), which settles a conflict by regenerating from the merged store - per directory rather than in the root file, so two areas migrating at once edit two different files. `src/templates/versus` is the first migrated area; `docs/metrics/2026-09-07-nested-contract-compiler.md` carries what it cost, which was 121 bytes MORE, and why the byte win has to come from the large areas instead. **Every re-measurement is its own file under `docs/metrics/`**, never an append to `docs/METRICS.md`, which stays the baseline: a shared append target is a guaranteed conflict between two rows running at once, and a conflict stops a landing dead.
...
- Hook and gate text: `rules.text(id)` from the store, so an executable rule's sentence has one home and the contract carries zero bytes for it once the named mechanism PRINTS it. *Corrected 2026-09-08: this used to be enforced as "once the named mechanism EXISTS", which is not the same claim. A carried rule is deleted from every loaded surface, and four rules written with `fires: test:<spec>` therefore vanished from the directory contracts and from `.claude/rules/` while their specs pinned the behaviour without ever saying the sentence. `scripts/contracts-lib.mjs` now proves the claim by looking for `rules.text('<id>')` in the mechanism's source, and `npm run learn` refuses a `fires:` whose mechanism does not carry it.*
...
- the retired owner rulings, the incident halves of `VERIFICATION.md` and `MISTAKE_TRIGGERS.md`, `incidents.md` and the memory `feedback` entries → records. `retired.json` and the negative check land here.
