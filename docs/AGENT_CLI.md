...
| `noacg bridge [--port 8899] [--origin URL ...] [--no-open] [--quiet] [--new-token]` | NoaCG Bridge, the ONE command here that is not about authoring a graphic (`docs/BRIDGE.md`). It holds an AMCP socket to CasparCG on behalf of the STUDIO PAGE, because a browser's only socket is a WebSocket and AMCP will never answer an HTTP Upgrade, and answers the page in playout terms - target, item, slot, verb - so the production page can put its output URL on a channel and cue the server's own templates and clips. Loopback-only bind, a stored per-machine token, an origin allowlist, and a `Host`-header check; `/health` is the one unauthenticated route so the panel can tell a missing Bridge from a rejected token; `/pair` spends the one-time code the link it opens carries. It lives in this CLI rather than as a second local helper, the same call `docs/PLAYOUT_INTEGRATION.md` §4 made for the exported package's relay, and `NoaCG-Bridge.exe` on each release is this command with Node inside (`cli/scripts/build-bridge-exe.mjs`). `noacg caspar agent` was its first name and still runs it. |
| `noacg caspar status\|send\|play\|stop [--server HOST] [--amcp-port 5250] [--channel 1] [--layer 20]` | The same AMCP with no browser in the loop at all - which is also the answer for Safari and any browser that will not let a page reach a local address. `play --url <output URL>` is the whole live link for NoaCG's own graphics: one `PLAY <channel>-<layer> [HTML] "<url>"`, after which every cue rides the durable command log. |
