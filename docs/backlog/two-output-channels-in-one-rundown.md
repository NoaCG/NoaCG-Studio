...
raised: 2026-09-23
state: unstarted
note: "2026-09-23: the owner made the per-cue channel choice the lead of this plan; docs/handoffs/2026-09-23-multichannel-playout.md hands it to the next session with every file named"
...
The owner's words on 2026-09-23, after the first real production: the channel per CUE is the
point. Settings channels exist so that choice is a short pick with a convenient default, never
the only place a channel is set. `docs/handoffs/2026-09-23-multichannel-playout.md` names the
files.

1. **Every cue carries its own channel, chosen like a CasparCG client does.** `PlayoutItem`
   gains an optional `channel`; absent means the graphics channel, so every saved show reads as
   before. The cue editor shows the channel beside the layer box, picked from the channels
   named in Settings, never typed, with a manual override per cue. `addPlayoutItem` gives a
   clip the INSERT channel when the studio has named one (and layer 10 there), and a template
   the graphics channel.
2. **Name the channels once, in Settings -> Playout.** Instead of one channel and one layer,
...
4. **All out clears every channel the rundown used**, not only the graphics one, and the
