...
raised: 2026-09-24
state: advanced
note: "Row F (claude/f-rewrite-skipped-specs, 2026-09-25) moved 160 tests back, 145 offline and 15 configured, onto Home, productions, the export window and the new editor; the configured floor is 46. Still missing: every test listed below, mostly ones whose subject was an old-editor panel or its preview frame."
...
The four helpers in `e2e/_create.ts` that reached the old editor - `enableAdvancedMode`,
`switchToAdvancedMode`, `finishIntoEditor` and `createProject` - call `skipOldEditor()`, which
skips the RUNNING test with a reason naming this file (`OLD_EDITOR_SKIP`). The configured suite's
`createGraphic` in `e2e/configured/_helpers.ts` reaches it through `finishIntoEditor`. So a test
skips when it calls one of them, and a rewritten test stops skipping once it stops calling them.

A test that was moved onto a new surface and did not pass there calls `skipOldEditor()` itself,
as the first line of its body, so it is listed below with the rest instead of failing.

## What it would take

For each test below, reach the same product behaviour without AppShell:

- **A graphic in the working slot** (`createProject`): call `bootstrapGraphic` instead, which
  seeds the same working graphic and lands on Home. From there `openProductionWithCurrent`,
  `openExportWindow` and `openWorkingGraphicInEditor` (all in `e2e/_create.ts`) open a
  production page, the export window and the new editor with its save controls. Most of what is
  left here failed on one of these, and the reason is in the failure: the old preview frame
  (`iframe.preview-frame`) and its Play button, the old stage and canvas, the dock tabs, or the
  timeline. Those need a real port, not a helper swap.
...
**2026-09-25, row F** (`claude/f-rewrite-skipped-specs`): the helpers above moved 160 tests back
into the suites, 145 offline and 15 configured, and their entries are gone from the list below.
Each offline one passed in its new home in a one-worker run before it was kept; every configured
one passed in a `configured-suite.yml` run dispatched on the branch. The configured floor
(`MIN_TESTS`) went from 31 to 46 and thirteen files left `ALLOWED_SKIPS`. The tests still listed
below are the ones that did not pass after the helper swap, and the ones nobody tried yet.
...
### `e2e/community.spec.ts` - 1 of 4 skipped

Era 5.5 community sharing.

- offline: no community affordances anywhere

### `e2e/competition-pack.spec.ts` - 11 of 12 skipped
...
### `e2e/control.spec.ts` - 5 of 11 skipped
...
### `e2e/exports.spec.ts` - 1 of 16 skipped
...
### `e2e/holding-pack.spec.ts` - 9 of 10 skipped
...
### `e2e/hosted-control.spec.ts` - 1 of 13 skipped
...
### `e2e/import-prepare.spec.ts` - 6 of 10 skipped
...
### `e2e/import-stretch.spec.ts` - 3 of 6 skipped
...
### `e2e/import-svg.spec.ts` - 47 of 98 skipped
...
### `e2e/inspector.spec.ts` - 8 of 9 skipped
...
### `e2e/library.spec.ts` - 7 of 22 skipped
...
### `e2e/production-audience.spec.ts` - 2 of 8 skipped
...
### `e2e/production-chat-intake.spec.ts` - 3 of 5 skipped
...
### `e2e/productions.spec.ts` - 3 of 23 skipped
...
### `e2e/render.spec.ts` - 1 of 10 skipped
...
### `e2e/template-pack-10.spec.ts` - 1 of 4 skipped
...
### `e2e/template-pack-4.spec.ts` - 2 of 10 skipped
...
### `e2e/configured/anonymous.spec.ts` - 2 of 10 skipped
