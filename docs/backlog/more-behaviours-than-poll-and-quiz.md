...
state: advanced
note: >-
  Two behaviours became fifteen recipes. 0040657c compiled the quiz through one binding
  ...
  bingo caller, and 2026-09-23 added the Standings, a ranking whose rows trade places as the
  points change (the `rank` kind and the PLACE paint rule). src/templates/behaviours/registry.ts
  is the list. The method the ask names is
  ...
  the survey's gaps are not all built - adjust-a-running-clock.md is the clearest one.
