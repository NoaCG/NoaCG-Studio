...
state: advanced
note: >-
  Level 1 is BUILT (2026-09-23, branch claude/noacg-work-suggestions-ibb9ak): the Standings
  recipe (src/templates/behaviours/ranking.ts) orders the rows through the runtime's `rank` field
  kind and moves each row's name and points into its new slot through a new PLACE paint rule, and
  the position numbers drawn in the slots renumber, sharing a place on a tie. The owner's own
  results-board.svg is proposed as Standings and walked in e2e/import-svg-behaviour.spec.ts; his
  walk is docs/acceptance/owner-queue/2026-09-23-standings-that-sort-themselves.md. STILL OPEN:
  levels 2 (external data drives it) and 3 (a control panel authored per graphic).
