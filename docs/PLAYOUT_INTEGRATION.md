...
| The channel shows "Failed to verify your browser" and "Vercel Security Checkpoint" instead of the graphic | Put on air worked; the hosting's bot check challenged CasparCG's built-in browser when it fetched the output page, and that browser cannot pass a challenge (seen in a real studio on 2026-09-23, checkpoint code 11) | On the noacg.studio deployment this is fixed once for everyone by a published firewall rule: Request Path starts with `/output`, action Bypass. A self-hosted deployment behind a bot check needs the same exemption for its output page. It is a site setting, never a per-studio one. |
