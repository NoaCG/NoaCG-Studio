...
| 14 | Point OBS, vMix or CasparCG at the output URL | WORKS WITH A STATED LIMITATION | OBS / vMix load `/output?production=<slug>` as a browser source (`src/output/main.ts`). CasparCG needs the local agent because a browser cannot open an AMCP socket: `noacg caspar agent` and `noacg caspar play --url` (`cli/src/commands/caspar.ts`, `src/control/casparLink.ts`, `e2e/caspar-connect.spec.ts` against a fake server, 15 CLI tests against a fake AMCP). On 2026-09-10 the Connect path ran against two real SERVERS - CasparCG 2.3.2 and 2.5.0, screen consumer: **Put on air** drove the agent, the agent drove AMCP, and the production aired, was updated live and came off again (`docs/acceptance/owner-queue/2026-09-10-bh-a-real-casparcg-has-now-run-the-output-url.md`). No SDI, no second machine, and the hosted origin's Local Network Access prompt still needs a person, so the hardware item (`docs/acceptance/owner-queue/2026-08-25-casparcg-connect-against-real-hardware.md`) stays open. The exported CasparCG package has been on hardware (owner, 2026-08-19, `docs/GOALS_ARCHIVE.md`). Note the origin trap: the button sends the output URL of the page it is pressed on, so pressing it on a dev server airs a bundle CasparCG 2.3.x cannot parse while every layer reports success. |
