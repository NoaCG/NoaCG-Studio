...
Branch `claude/bj-published-path-lag`, on the worktree `agent-a901bde5ea093cca0`. Four commits,
build green, CI green with the E2E shards correctly skipped (this branch touches no `src/`).
...
`ambientEnv`, which ALREADY falls back to the main checkout for exactly this and has since it was
written. Nothing was ever the owner's here.
...
one contaminated row, its family median unchanged. Both stamps are now matched to the verb.
...
- `scripts/playout-wire-probe.mjs` - the two wire hops in fifteen seconds, no browser, no dev
  server, no job slot. The instrument to reach for first, and the one to run at a venue.
