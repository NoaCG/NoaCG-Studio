...
| `SVG_BEHAVIOUR_PLAN.md` | IMPLEMENTATION PLAN, 2026-09-05: one binding format for ANY behaviour on imported artwork, replacing the five hand-written modules. Roles stamped in the markup plus a versioned `NOACG_BEHAVIOUR` table read by one paint runtime; recipes as declarations (the shipped five become rows, `switch` and `choice` become the universal floor); conditions as picks from states and field facts, with field kinds owning every comparison; the ladder from "import and done" to the sentence board, and where the binding stops. Eight worked examples across graphic types, the migration rules, and a seven-phase rollout - phases 0-2 are current work, phase 3 on is proposed under P2. |
| `SVG_BEHAVIOUR_SHOWS.md` | THE REUSE TEST, run 2026-09-06: what American game shows and late-night talk shows put on air with operator commands, the six graphics picked to test the behaviour system, the honest first import of each, and the outcome (five recipes of the shipped shape and three field kinds built; the bracket binds as data; two findings recorded in the plan's section 13). |
...
| `LANDSCAPE.md` | The map ABOVE the per-vendor matrix, read 2026-09-07: the market divided into seven layers with the buyer and the price of each, one unified capability matrix across all of them with every NoaCG state verified against this repository, where we are genuinely ahead by the strict test (a rival would have to rebuild their architecture), where we are exposed ordered by how often it bites, and the ordered requirements list for an all-around broadcast tool in four tiers down to an argued NEVER. Section 6 makes the strongest case AGAINST becoming an all-around tool and then answers it: all-around reach, not all-around scope. |
| `COMPETITOR_MXMZ.md` | RESEARCH, 2026-08-22 and re-read 2026-08-28, public sources only: what MXMZ (the closest working model) actually ships - the Illustrator/Figma → SVG → browser keyframe timeline → one-URL-per-channel workflow, their prices and customers - and the finding that shapes our own road: nothing public shows them AUTHORING logic. They train the designer for a day, keep the operator as the non-technical one, and hand-build a control panel per sport. **Section 8 is the correction**: that finding covers authored logic ONLY, and on the AI question the competition is an assembly layer above the graphics engine (HighField AI, which names MXMZ as one of four engines it drives) rather than anyone generating a graphic. What to take, what not to. |
| `COMPETITOR_PIXLA.md` | RESEARCH, read 2026-09-07, public sources only: Pixla, a control room in a box (rundown, prompter, stage timer, media library, TriCaster automation, scheduler, 3D studio designer) with an OGraf-branded motion designer inside it, at EUR 19/89 a month with a demo-data-only free tier. Two grades of evidence and the file marks which: their marketing, and their **shipped client code**, which corrects the marketing four times - their designer emits no OGraf v1 package artefact, ATEM appears in their page title and zero times in their controller, they touch no part of the open playout stack, and their NDI path is Electron-only. Carries the full capability matrix, the argued NOW/NEXT/LATER/NEVER plan, and the positioning line. The refusals are the point: most of their list is deliberately not ours. |
| `COMPETITORS.md` | The per-vendor capability matrix: MXMZ, Pixla, Singular.live, Loopic, Zero Density and Rive, plus the OGraf convergence block, each capability marked they-have / we-match / we-beat / GAP, with UNRESEARCHED said out loud where nobody has looked. Mostly assembled from the docs below rather than from new research; refreshed on TIME, not on commits. A GAP row is candidate work - the largest one is assembly from newsroom context. `LANDSCAPE.md` is the map above it, and is where the "what should we build" question is answered. |
...
| `WORKFLOW_ARCHITECTURE.md` | How development works now, for an outside developer or a fresh session: parallel sessions in their own worktrees, instructions compiled from `contracts/rules/` and how Claude Code and Codex load them, learning, verification, landing through GitHub's merge queue, the CI validation tiers, the machine job queue, the orchestrator, and the commands that measure it. The 2026-09-06 diagnosis and staged plan it replaced are in git history. |
| `METRICS.md` | The 2026-09-06 baseline for every target the workflow plan set, with the `npm run metrics:*` command that reproduces each number. Each later re-measurement is its own file under `docs/metrics/`, so a row that measures something conflicts with nobody. |
...
| `OGRAF_FIRST_REVIEW.md` | RATIFIED 2026-08-29 with four amendments: what the standard does and does not give us. Its code-as-truth authoring model is superseded by `GOALS.md`: OGraf is the long-term interoperability direction, not a declared internal format. |
...
| `ORCHESTRATOR_STACKS_AND_HARNESS_REVIEW.md` | 2026-09-17: GitHub's native stacked pull requests refused as a wave mechanism and adopted as a queue guard, with the measured cost of a chained wave; the September harness releases judged feature by feature - background sessions instead of Agent-tool rows, schema-enforced delegated results, and what is already covered. |
...
| `SPX_EXAMPLES_CORPUS.md` | Research notes on `spx_examples/` - ~1.3 GB of real SPX productions from a Finnish public broadcaster and orchestras. Gitignored, never committed; this is what reading it taught. |
