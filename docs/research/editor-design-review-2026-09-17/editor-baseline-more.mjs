...
const report={at:new Date().toISOString(),actions:[],gestures:[]};const save=()=>writeFileSync(out+'/baseline-additional.json',JSON.stringify(report,null,2));
try{
 await server(project,project,5291,offline);await server(studio,studio+'/apps/editor',5292);browser=await chromium.launch();
 const page=await browser.newPage({viewport:{width:1366,height:768}});page.setDefaultTimeout(12000);await page.goto('http://127.0.0.1:5291/app#/home');
 try{
  await page.getByTestId('home-new-project').click();await page.locator('[data-entry="import-graphic"]').click();
  await page.locator('.wz-drop input[type=file]').setInputFiles(project+'/docs/svg-samples/scorebug.svg');
  await page.getByTestId('import-svg-card').waitFor();await page.locator('.wz-modal').getByRole('button',{name:'Next →',exact:true}).click();
  await page.getByTestId('map-svg-fields').waitFor();report.actions.push({name:'default SVG mapping',text:await page.locator('.wz-modal').innerText()});
  for(let i=0;i<6&&!(await page.getByTestId('wz-finish-name').count());i++)await page.locator('.wz-modal').getByRole('button',{name:'Next →',exact:true}).click();
  await page.getByTestId('wz-finish-name').fill('Baseline scorebug');report.actions.push({name:'default SVG finish',editorDoor:await page.getByTestId('wz-finish-editor').count()});
  await page.screenshot({path:out+'/noacg-svg-finish.png'});
  await page.getByTestId('wz-finish-production-name').fill('Baseline laboratory');await page.getByTestId('wz-finish-production-go').click();await page.getByTestId('wz-finish-production-confirm-go').click();
  await page.getByTestId('verb-take').waitFor({timeout:30000});
  const template=await page.evaluate(async()=>{const{useTemplateStore}=await import('/src/store/templateStore.ts');return useTemplateStore.getState().template});
  writeFileSync(out+'/f2-scorebug.json',JSON.stringify(template,null,2));
  report.actions.push({name:'SVG into production',url:page.url(),templateName:template.name,text:await page.locator('body').innerText()});
  await page.getByTestId('verb-take').click();await page.waitForTimeout(900);await page.screenshot({path:out+'/noacg-svg-production.png'});
  await page.reload();await page.getByTestId('verb-take').waitFor();report.actions.push({name:'saved SVG production reload',url:page.url(),text:await page.locator('body').innerText()});
 }catch(e){report.actions.push({name:'SVG route observation stopped',error:String(e),text:await page.locator('body').innerText()});}save();await page.close();
 for(const width of[1366,1920])for(const name of['noacg','studio']){
  ...
  try{
   ...
   for(let i=0;i<10;i++){
    const target=name==='noacg'?p.locator('.tlv2-labels .timeline-label[data-part="#stress-0"]'):p.locator('.timeline-layer-select[title="Select Layer 1"]');await target.click();if(name==='noacg'){const b=await p.getByTestId('tlv2-clip-0').boundingBox();await p.mouse.click(b.x+40,b.y+8);await target.click();await p.getByTestId('inspector-part-label').waitFor();}await p.waitForTimeout(300);
    let x,y;
