# The contract src/blocks/AGENTS.md carried before it was compiled
...
the helper the mechanism actually lives in, the spec that pins it, the identifier you would grep
for.

Nothing loads this. It is the evidence behind sixty-odd rules at once, in the same way a record
...
Three of its claims were already false when this was taken, and none was migrated as written.
...
1. **"The stub applies `fullscreen` and nothing else, so an entry here is only as alive as its
   ...
   `bug`, `logo` - and `generate` applies `fullscreen`. Believing the contract would have licensed
   deleting seven live entries as unreachable. It also hid a real defect: there is no block with the
   id `lower-third`, so `block('lower-third')` returns undefined and the non-null assertion in
   `stubProvider.ts` throws the moment a prompt says "lower third".
2. **"Fixed contracts (scoreboard cells, quiz rows) and data-driven categories (hidden textarea
   ...
   and REFUSES with an explanation when none can land a real element - a field nothing on screen
   answers is exactly the defect `scripts/field-coverage.mjs` exists to catch. The same stale
   sentence appears twice more, in the `designLayout.ts` section ("they return null off-shape so the
   caller falls back to the definition-only add") and in the doc comments of `blocks/edit.ts` and
...
3. **"presetRegistry.ts ... Two callers, one shape."** There are three. Besides `presetApply` and
   ...
   is the caller most worth naming: the worked example is the motion the model is asked to imitate,
   so a change here changes what every generated graphic looks like.

`docs/metrics/2026-09-07-blocks-migrated.md` is the full report: what became a rule, what was
false, and what is true but no longer worth loading.
...
- **animData.ts** - the schema + the literal's read/write. AnimData = version/root/speed/steps
  **/machine?** (the STATE MACHINE, format version 2 - docs/STATE_MACHINE_SCHEMA.md);
  ...
  label/section/order/payload-field-ids/adjust-deltas/set-figures/add-and-remove-lists/destructive per
  operator event (one road per field; `add`/`remove` map a lines field to the field whose value is
  the line), travelling INSIDE the template so exported control pages keep their labels
