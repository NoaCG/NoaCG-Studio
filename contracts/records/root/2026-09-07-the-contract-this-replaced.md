# The root AGENTS.md as it stood before it was compiled

**Recorded 2026-09-07.** The file verbatim at `origin/main`, 23,436 bytes read by fifty-four
instruction chains - by a wide margin the most expensive file in the repository.

Kept because rules written in new words lose the exact symbol names the prose carried, and
`npm run contract:migrate -- audit` reads it when checking the migration lost nothing.

**It is history, not a contract.** Where it disagrees with a rule in the store, the rule is right.
What went where, and what deliberately did not become a rule, is in
`docs/metrics/2026-09-07-root-migrated.md`.
