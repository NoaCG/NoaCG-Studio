# The contract src/model/AGENTS.md carried before it was compiled

**Recorded 2026-09-08**, the day the area moved into the rule store. This is the file verbatim as
...
the helper the mechanism actually lives in, the module a number was measured with, the identifier
you would grep for.

Nothing loads this. It is the evidence behind fifty-odd rules at once, in the same way a record
holds the evidence behind one, and `node scripts/contract-migrate.mjs audit` reads it when checking
that the migration lost nothing the contract knew.

**It is history, not a contract.** Where it disagrees with a rule in the store, the rule is right.
Seven of its claims were already false or stale when this was taken, and none was migrated as
written. Each is quoted with the code that disagrees.

1. **"`graphics` order IS the layer stack, in PAINT order - index 0 furthest back, the last entry
   on top."** It is not, and has not been since a pool graphic gained a layer NUMBER the operator
   types (`DEFAULT_PLAYOUT_LAYER`, `nextFreeLayer`). Every z-order consumer reads `graphicLayer(g)`
   instead: `src/control/hostedControl.ts`, `src/export/showExport.ts`, `src/packs/graphicsPack.ts`
   and `src/components/home/ProductionPage.tsx`. `shows.ts` itself says so twenty lines away - "a
   pool graphic airs on a layer NUMBER the operator types, not on one derived from its position in
   the pool". This was the most dangerous claim in the file: a session believing it would have
   reordered an array to change what airs on top, and nothing would have moved.
2. **"Both surfaces that show it reverse it for display, because a layer panel lists the front
   first; `moveShowGraphic(+1)` therefore means forward."** Neither surface reverses anything. The
...
3. **"The alias table is THREE declared tables - `ALIASES_EN` / `ALIASES_SV` / `ALIASES_FI`."**
   ...
   phrases reach search automatically. The rule that replaced it carries no count.
4. **"`ensureFontFace` ... every path that can retarget a typeface calls it: `templates/shared/
   base.ts` at build, ..."** `ensureFontFace` has no reference anywhere under `src/templates/`.
   `base.ts` imports `fontFaceCss` and returns the face CSS with the stack - it covers the build's
   own faces by EMITTING them, not by registering them. The other three paths named are real. The
   distinction is the whole rule: the guarantee is needed where a variable is retargeted AFTER the
   build, which is exactly why the wizard's `cssVarOverrides` is on the list.
5. **"`accentInk` is `var(--panel-bg)` in three families and a LITERAL dark in glass - the other
   three panel on a near-black"**, and, three sentences earlier, **"with values for all four
   `StyleTag` families"**. `FAMILY_TOKENS` carries six families; five resolve the ink to
   `var(--panel-bg)` and glass is the one literal. The same stale count sat in `themeTokens.ts`'s
   own comment above the glass block, and was corrected on the way in.
6. **"The rule for admitting a new one is written next to the list and GATED by
   `validateTaxonomy`."** The list and the prose are in `taxonomy.ts`; the gate is not.
   `MAX_OCCASIONS`, `MIN_DESIGNS_PER_OCCASION` and the check that every declared phrase resolves
   live in `src/templates/templateMeta.ts`. A reader who went looking for the ceiling where the
   contract said it was would not find it.
7. **"a provider that simply DIDN'T re-declare its inputs must send `null`, not `[]`, or the merge
   empties the user's Content panel."** The provider contract is right and the mechanism is
   misplaced: `mergeVideoInputs(prev: VideoInput[], next: VideoInput[])` cannot receive `null`. The
   decision lives at the call site, `src/components/video/VideoAiChatPanel.tsx` - `result.inputs ?
   mergeVideoInputs(p.inputs, result.inputs) : p.inputs`. The rule now names where it is enforced.

Two smaller drifts, both harmless and both corrected in the rules: the sync conflict copy strips
five fields, not the three the contract listed (`joinSlug` and `presenterSlug` joined the hosted and
output slugs and `publishedAt`), and `src/model/wizard.ts` has 620 importing files under `src/`, not
619. Six modules had no bullet at all - `googleFonts.ts`, `googleFontsIndex.ts`, `storageHealth.ts`,
`structuralIntent.ts`, `templateSet.ts` and `templateVocabulary.ts` - so no audit would have noticed
them missing; they are named in `docs/metrics/2026-09-08-model-contract-migrated.md`.
