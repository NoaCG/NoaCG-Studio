...
function bytesAt(sha, file) {
  return Buffer.byteLength(git(['show', `${sha}:${file}`]).replace(/\r\n/g, '\n'), 'utf8');
}
...
function chains(agentsFiles, sizes) {
  return agentsFiles.map((leaf) => {
    ...
    const parts = agentsFiles.filter((f) => {
    ...
    return { leaf: dir === '.' ? '(root)' : dir, parts: parts.length, bytes };
  });
}
...
function main() {
  ...
  for (const leaf of agents) {
    ...
    for (const f of agents) {
      const d = path.posix.dirname(f);
      if (d === '.' || dir === d || dir.startsWith(`${d}/`)) loads.set(f, loads.get(f) + 1);
    }
  }
  ...
  for (const c of allChains.slice(0, 5)) console.log(`    ${String(c.bytes).padStart(8)}  ${c.parts}  ${c.leaf}  (~${Math.round(c.bytes / 4).toLocaleString()} tokens)`);
