...
the branch tip's age, and the mtime of the session's transcript. **The transcript is blind during one
pending tool call**: a silent transcript whose last tool call has no result is a call still running,
not a dead session - on 2026-09-11 the loop called row CA dead after two quiet hours while it sat
inside one browser call, and it came back hours later. **`blocked-sessions.mjs` is not
